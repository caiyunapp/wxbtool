...
def test_model(mdl):
    ...
    def test(mdl):
        ...
        dataloader = DataLoader(mdm.dataset_test, batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)
        loss_per_epoch = 0.0
        ...
        for step, sample in enumerate(dataloader):
            ...
            with th.no_grad():
                ...
                eva.weight = eva.weight.reshape(1, 1, 32, 64)
                eva.weight = eva.weight * (eva.weight > 0)
                rmse = eva.weighted_rmse(rst, tgt)
        ...
        rmse_total = rmse_per_epoch_t / mdm.test_size
        logger.info(f'Test Loss: {loss_per_epoch / mdm.test_size}')
